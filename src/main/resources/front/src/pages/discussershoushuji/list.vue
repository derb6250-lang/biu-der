		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('discussershoushuji','新增')" type="primary" @click="add('/index/discussershoushujiAdd')">
				...
				<el-table class="tables" :stripe='false'
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="table-del" type="success" @click.native="delClick(scope.row)" v-if="btnAuth('ershoushuji','删除评论')">
								<span class="icon iconfont icon-shanchu6"></span>
								删除
							</el-button>
							<el-button class="table-btn1" type="success" @click.native="discussClick(scope.row)" v-if="btnAuth('discussershoushuji','查看评论')">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`discussershoushuji/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/discussershoushujiDetail', query: params});
			},
			...
			discussClick (row){
				...
				this.$router.push({path: '/index/discussdiscussershoushuji', query: params});
			},
			editClick (row){
				this.$router.push(`/index/discussershoushujiAdd?type=edit&&id=${row.id}`);
			},
			async delClick (row){
				await this.$confirm('是否删除此评论？') .then(_ => {
					this.$http.post('discussershoushuji/delete', [row.id]).then(async res => {
						if (res.data.code == 0) {
							await this.$http.get('ershoushuji/info/' + row.refid).then(async rs=>{
								rs.data.data.discussnum--
								await this.$http.post('ershoushuji/update',rs.data.data).then(rs2=>{})
							})
							this.$message({
								message: "操作成功",
								type: "success",
			...
			discussistopChange(e,row){
				this.$http.post('discussershoushuji/update',row).then(res=>{})
			},
		}
	}
</script>
