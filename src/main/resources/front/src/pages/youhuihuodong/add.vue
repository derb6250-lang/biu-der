...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="活动日期" prop="huodongriqi">
				<el-date-picker
					:disabled="ro.huodongriqi"
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					this.ruleForm.huodongmingcheng = '优惠活动'; 				}else if(type=='edit'){
					this.info()
				}
				// 获取用户信息
			...
			// 多级联动参数
			info() {
				this.$http.get(`youhuihuodong/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`youhuihuodong/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
