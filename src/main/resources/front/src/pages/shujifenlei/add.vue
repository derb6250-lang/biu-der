		...
		<el-form
			...
			<el-form-item class="add-item" label="羽毛球装备分类" prop="shujifenlei">
				<el-input v-model="ruleForm.shujifenlei" 
					placeholder="羽毛球装备分类" clearable :readonly="ro.shujifenlei"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`shujifenlei/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shujifenlei/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
